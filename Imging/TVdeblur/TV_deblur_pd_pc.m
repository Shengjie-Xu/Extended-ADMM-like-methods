...
function [u,PSNR,Time,Itr] = TV_deblur_pd_pc(x0,h,opts,I)
% the primal-dual extended ADMM for image deblurring with total variation
nu=0.99;
[n1,n2,n3] = size(x0);
beta = opts.beta;
mu   = opts.mu;
...
for k =1:MaxIt    
    % prediction step
    % u-update
    Temp= PTx(beta*v1+lbd1) + PTy(beta*v2+lbd2) + HTx0;
    ...
    v11= Px(un);
    v22= Py(un);
    sk1 = vn1+v11-v1 - lbd1/beta;
    sk2 = vn2+v22-v2 - lbd2/beta;
    nsk = sqrt(sk1.^2 + sk2.^2); nsk(nsk==0)=1;
    nsk = max(1-1./(beta*nsk),0);
    vn1n = sk1.*nsk;
    vn2n = sk2.*nsk;        
    % lambda-update
    lbd1nn = lbd1 - beta*(v11 - vn1n);
    lbd2nn = lbd2 - beta*(v22 - vn2n);  
    % correction
    lbd1 = lbd1nn + nu*beta*(v1-v11);
    lbd2 = lbd2nn + nu*beta*(v2-v22);
