function [xave, history] = linear_svm_pd_pc(A, lambda, p, rho)
...
for k = 1:MAX_ITER
    ...
    for i = 1:N
        cvx_begin quiet
            variable x_var(n)
            minimize ( sum(pos(A{i}*x_var + 1)) + rho/2*sum_square(x_var -x(:,i)-u(:,i)/rho) )
        cvx_end
        xx(:,i) = x_var;
    end
    xave = mean(xx,2);
    % z-update 
    zold = z;
    z = N*rho/(1/lambda + N*rho)*mean( xx-x+zold - u/rho, 2 );
    zz = z*ones(1,N);
    % u-update
    uu = u - rho*(xx - zz);
    % correction step
